...
/*	map<int, Table_change*>::iterator iter;
	...
	for(i=1;i<=SQ.join_count;i++)
	{
		...
		for(iter=_query_plan_tree.tree_tablename.begin();iter!=_query_plan_tree.tree_tablename.end();iter++)
		{
			...
			if(getfirsttablename(iter->second) == SQ.JoinList[i-1].tb_name1)
			{
				...
				if(getlength(iter->second) == 1) //haven't been joined  ���û�б�join����ֱ���޸�child��ֵ�����޸ı������ 
				{
					_query_tree_node[num].child[0] = iter->first;
					
					Table_change* temp2;
					Table_change* p = iter->second; //��ĩβ���������������ֺ�������Ǳ��ĸ�index�޸ĵ� 
					while(p){
						temp2 = p;
						p = p->next;
					}
	...
	for(i = 1;i <= SQ.cond_count;i++)
	{
		...
		_query_tree_node[num].index = num;
		_query_tree_node[num].child[0] = num-1;//��������һ�ε�join�����Ľ�β 
		_query_tree_node[num].child[1] = 0; //�Һ���Ϊ��
...
/*	for(i = 1;i <= tree->treenodenum;i++)
	...
	num++;
	for(itertable = tree->tree_tablename.begin();itertable != tree->tree_tablename.end(); itertable++)  // ����union�ڵ� 
	{
...
void how_to_join(SelectQuery &SQ, Query_Plan_Tree*  tree, TableList* tblist)
{
	...
	for(iterjoin = tree->tree_join.begin(); iterjoin != tree->tree_join.end();iterjoin++)
	{
		...
		if(flag1==1 && flag2==1)
		{
			...
			if(whether_joinnode_all_child_areunion(tree,parent1) == 1)  //û�п��������������join��1�ڵ���ô�죡�� 
			{
	...
	//�ж��ǲ���h�ֲ��� ��������������Ӷ���union�ڵ㣬��ô�㿴child[1],��ȥ��child0 
	//cout<<"come to here"<<endl;
	for(iterjoin = tree->tree_join.begin(); iterjoin != tree->tree_join.end();iterjoin++)
	{
...
// void xiancheng(int n22,string ip1,int port1,string db_name1,string stmt1)
// {
// 	// extern int *count_ptr;
// 	// cout<<*count_ptr<<endl;
//     cout << "thread " << n22 << "executed" << endl;

	
//     RPCExecute(ip1,port1,db_name1,stmt1);
//     cout << "thread " << n22 << "end" << endl;
//     *count_ptr = *count_ptr + 1;
// 	return;
// }
// void xiancheng1(int n22)
// {
// 	// extern int *count_ptr;
// 	cout<<*count_ptr<<endl;
//     cout << "thread " << n22 << "executed" << endl;
...
//     *count_ptr = *count_ptr + 1;
// }
void f(string ip,int port,string db_name,string stmt,int *count){
 cout << "no" << *count << "start" << endl; 
 RPCExecute1(ip,port,db_name,stmt);
 *count = *count + 1;
 cout << "no" << *count << "end" << endl; 
}
void generate_sql_list(SelectQuery &SQ, Query_Plan_Tree*  tree, TableList* tblist)
{
	map<int, Join_Middle_Table>::iterator iter;
 	for(iter = join_middle_table.begin();iter!=join_middle_table.end();iter++)
 	{
  		join_middle_table.erase(iter);
 	}
	for(int ww = 0; ww < SQ.from_count; ww++)
 	{
  		cout<<"my table name:"<<SQ.FromList[ww].tb_name<<"   ";
 	}
 	cout<<endl;
	//map<int, Table_change*> tree_tablename;
	...
	for(iterjoin = tree->tree_join.begin();iterjoin != tree->tree_join.end();iterjoin++)
	{
		...
		if(tree->query_tree_node[lchildindex].type == 4 && tree->query_tree_node[rchildindex].type == 4){
			cout<<"all join num is "<<iterjoin->second.joinnum<<endl;
 			// extern int N;
			// extern int *count_ptr;
			// N=iterjoin->second.joinnum;
			// cout<<"观察N的值"<<N<<endl;
			// *N1 = iterjoin->second.joinnum;
			int count = 0;
			...
			if(tree->query_tree_node[lchildindex].type == 4 && tree->query_tree_node[rchildindex].type == 4) //the join node 's children are all tables
			{
				...
				cout<<insertsql<<endl;
				cout<<"send it to db "<<iterjoin->second.join_frag[j].cast_to_db_id<<endl;
				// thread t(xiancheng,j,sites[iterjoin->second.join_frag[j].cast_to_db_id-1],select_ports[iterjoin->second.join_frag[j].cast_to_db_id-1],dbs[iterjoin->second.join_frag[j].cast_to_db_id-1],insertsql);
				// thread t(xiancheng1,j);
				cout<<"我要开启线程了"<<endl;
				// this_thread::sleep_for(chrono::milliseconds(100*1000));
        		// t.detach();
			...
			while(count != iterjoin->second.joinnum){
		...
		//{
		//	join_table.newtable->fraginfo[r] = tblist->tbl[staytableindex]->fraginfo[r]; //����Ӧ������Ū�����Ǹ������join_table;�����Ҿ�ȫ��join�ˡ� 
	//	}
	...
	int ffflag1=0,ffflag2=0,ffflag3=0;
	for(int ww = 0; ww<SQ.from_count;ww++)
	{
		if(SQ.FromList[ww].tb_name == "Course") ffflag1 = 1;
		if(SQ.FromList[ww].tb_name == "Exam") ffflag2 = 1;
		if(SQ.FromList[ww].tb_name == "Student") ffflag3 = 1;
	}
		...
		for(iterjoin = tree->tree_join.begin();iterjoin!=tree->tree_join.end();iterjoin++)
		{
			...
			if(tree->query_tree_node[parent].type == 2)
			{
				...
				for(int q = 0;q < MAX_DB_COUNT;q++)
				{
					...
					{
						if(ffflag1==1 && ffflag2==1 && ffflag3==1)
						{
							sql = "select distinct ";
						}
						else if(ffflag1==0 || ffflag2==0 ||ffflag3==0) sql = "select ";
						for(iterproject = tree->tree_project.begin();iterproject!=tree->tree_project.end();iterproject++)
						{
							sql = sql + iterproject->second.project.table_name + "_" + iterproject->second.project.col_name+",";
						}
						sql = sql.substr(0,sql.length()-1);
						cout<<"the new table name is:"<<iterjoinmiddle->second.newtablename<<endl;
