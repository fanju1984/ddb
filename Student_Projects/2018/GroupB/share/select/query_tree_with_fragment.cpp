...
void find_all_attr_needed(SelectQuery &SQ,string tablename,string attr[])
{
	int i,j;
		...
		if(itertable->second->whether_have_fragment == 1)
		{
			...
			else
			{
				while(tree->query_tree_node[par].type == 1)
				{
					...
					find_all_attr_needed(SQ,getfirsttablename(itertable->second),_opera_selpro.attr);
...
void cut_useless_vertical_node(SelectQuery &SQ, Query_Plan_Tree*  tree, TableList* tblist)
{
	...
	for(i=0;i<tblist->table_num;i++) //find the table vertical fraged
	{
		if(tblist->tbl[i]->frag_type == 1)
		{
			...
			if(fflag == 0) return;
			find_all_attr_needed(SQ,tablename,attrneeded);
...
string print_condition(TableList* tblist,Condition condition)
{
	string temp = "";
...
string print_selpro_create_table(Query_Plan_Tree*  tree,TableList* tblist,string tablename,int frag_id,string attrneeded[],bool whether_join = false)
{
...
void generate_sql_list(SelectQuery &SQ, Query_Plan_Tree*  tree, TableList* tblist)
{
	...
	for(iterfrag = tree->tree_frag.begin();iterfrag!=tree->tree_frag.end();iterfrag++)
	{
		...
		//	cout<<"send it to DB "<<db_id<<endl;
			cout << "sql:::" << sql << endl;
			db_id = iterfrag->second.tbl->fraginfo[iterfrag->second.frag_id-1].db_id;
			// result = RPCExecute(sites[db_id-1],dbs[db_id-1],sql);
		...
		else if(tree->query_tree_node[parent].type == 6 && iterfrag->second.tbl->frag_type==1)
		{
			...
			lasttablename = get_last_tablename(iterfrag->second.tbl->tb_name,iterfrag->second.frag_id);
			sql = "create table "+ lasttablename + " as select * from "+iterfrag->second.tbl->tb_name+";";
			cout<<sql<<endl;
			db_id = iterfrag->second.tbl->fraginfo[iterfrag->second.frag_id-1].db_id;
			cout<<"send it to DB "<<db_id<<endl;
			bool result = RPCExecute(sites[db_id-1],dbs[db_id-1],sql);
   			if(!result) {cout<<"ERROR: here 1 failed"<<endl;}
   			cout<<"result is "<<result<<endl;			
		}
	}
	...
	for(iterjoin = tree->tree_join.begin();iterjoin != tree->tree_join.end();iterjoin++)
	{
		...
		if(tree->query_tree_node[lchildindex].type == 4 && tree->query_tree_node[rchildindex].type == 4){
			...
			if(tree->query_tree_node[lchildindex].type == 4 && tree->query_tree_node[rchildindex].type == 4) //the join node 's children are all tables
			{
				...
				{
					string attrneeded[MAX_ATTR_COUNT]={""};
					find_all_attr_needed(SQ,iterjoin->second.join_frag[j].tbl_to_broadcast,attrneeded);
					sql = print_selpro_create_table(tree,tblist,iterjoin->second.join_frag[j].tbl_to_broadcast,frag_to_broadcast_table_id,attrneeded);
					cout<<sql<<endl;
				...
				find_all_attr_needed(SQ,iterjoin->second.join_frag[j].tbl_to_broadcast,table_broadcast_attrneeded);
				string table_stay_attrneeded[MAX_ATTR_COUNT]={""};
				find_all_attr_needed(SQ,iterjoin->second.join_frag[j].tbl_to_stay,table_stay_attrneeded);
				
				sql += "("+print_selpro_create_table(tree,tblist,iterjoin->second.join_frag[j].tbl_to_broadcast,frag_to_broadcast_table_id,table_broadcast_attrneeded,true);
				sql += "," +print_selpro_create_table(tree,tblist,iterjoin->second.join_frag[j].tbl_to_stay,frag_to_stay_table_id,table_stay_attrneeded,true)+");";
				cout<<endl<<sql<<endl;
				tempattrs = print_selpro_create_table(tree,tblist,iterjoin->second.join_frag[j].tbl_to_broadcast,frag_to_broadcast_table_id,table_broadcast_attrneeded,true) +  
					"," +print_selpro_create_table(tree,tblist,iterjoin->second.join_frag[j].tbl_to_stay,frag_to_stay_table_id,table_stay_attrneeded,true);
		...
		else if(tree->query_tree_node[lchildindex].type != 4 || tree->query_tree_node[rchildindex].type != 4)
		{
			...
			//
				...
				if(iterjoin->second.join_frag[j].from_db_id != iterjoin->second.join_frag[j].cast_to_db_id)
				{
					string attrneeded[MAX_ATTR_COUNT]={""};
					find_all_attr_needed(SQ,iterjoin->second.join_frag[j].tbl_to_broadcast,attrneeded);
					sql = print_selpro_create_table(tree,tblist,iterjoin->second.join_frag[j].tbl_to_broadcast,iterjoin->second.join_frag[j].from_db_id,attrneeded);
					cout<<sql<<endl;
					cout<<"send it to db "<<iterjoin->second.join_frag[j].cast_to_db_id<<endl;
					cout<<"insert into table"<<endl;
				}
				...
				string table_broadcast_attrneeded[MAX_ATTR_COUNT]={""};
				find_all_attr_needed(SQ,iterjoin->second.join_frag[j].tbl_to_broadcast,table_broadcast_attrneeded);
				string uniondesc = print_selpro_create_table(tree,tblist,iterjoin->second.join_frag[j].tbl_to_broadcast,iterjoin->second.join_frag[j].from_db_id,table_broadcast_attrneeded,true);
				sql = sql + uniondesc;
	...
	if(SQ.join_count == 0)
	{
		if(SQ.from_count==1 && SQ.cond_count==0)
		{
		...
		else if(SQ.from_count==1 && SQ.cond_count!=0)
		{
