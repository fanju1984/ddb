...
class Attribute:
    ...
    table         table name
    attr          attribute name
    f_id          fragment id if this attribute is a condition for vertical fragment
    if_join_attr  if this attribute is included in a join condition
    """
...
class Selection:
    """
    """
    def __init__(self, attr: Attribute, operator: str, f_id: int, s_id:int, num_value: float=None, str_value: str=None):
        self.attr = attr       #attribute name
        self.operator = operator       #operator > = < >= <=
        self.f_id = f_id             #fragment id if this select is a condition for horizontal fragment, start from 0
        self.s_id = s_id           #site id if this select is a condition for horizontal fragment, start from 1, 1,2,3,4
...
class Fragment:
    ...
    def __init__(self, id:int, table: str, s_id: int, size: int, hor_or_ver: int, hori_condi: List[Selection]=None, 
                  verti_condi: List[Attribute]=None):
...
class Table:
    ...
    def __init__(self, table:str, frag_list:List[Fragment], attributes:List[str],distri_attr: Attribute=None,
                 distri_type: int=None, distri_bound: tuple=None):
        self.table=table                 #table name
...
class Node:
    ...
    def __init__(self, index: int, type:str, parent:int, children:List[int], tables:List[str], 
                 site:int, size:int, columns:List[str], if_valid:bool=True, f_id:int=None, 
                 f_name:str=None, select_condi: List[List[str]]=None, 
                 projection:List[str]=None, join:List[str]=None, top:int=None):
        self.id = index                   #node id, start from 0
        self.type = type                   #type fragment, select, projection, join, union
        self.parent = parent             #the parent node id, default -1
        self.children = children         #the children node ids, defalut []
        self.tables = tables             #table names in this node
        ...
        self.join = join                 #if type is join, the join condition
        self.top = top                 #if type is 'fragment', correspond id for join