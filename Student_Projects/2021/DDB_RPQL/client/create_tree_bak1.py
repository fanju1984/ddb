...
def create_a_tree(sql):
    ...
    for t in range(len(tabless)):
        ...
        for i in etcd.get_prefix("/table/" + tabless[t] + "/fragment"):
            fragnum += 1
            sname = i[1].key.decode('utf-8').replace('/table/' + tabless[t] + '/fragment/','')
            ...
            if finfo["conditions"]!='':
                ...
                for comp in conditionlist:
                    ...
                    if right.isdigit():
                        ...
                        hconds.append(Selection(compattr, op, f_id, sname2sid[sname], num_value = int(right)))
                    else:
                        #print('Selection',compattr.table,compattr.attr,op,f_id,s_id,right)
                        hconds.append(Selection(compattr, op, f_id, sname2sid[sname], str_value = right))
            else:
                for at in attrl:
                    if at.attr in finfo['columns']:
                        vconds.append(at)
            if finfo["conditions"]!='':
                lenrow = int(etcd.get("/table/" + tabless[t] + "/lenfragment/"+sname)[0])
                rowsize = 0
                ...
                frlist.append(Fragment(f_id,tabless[t],sname2sid[sname],lenrow*rowsize,0,hori_condi=hconds))
                ...
                for c in finfo['columns']:
                    #print("/table/" + tabless[t] + "/columnsize/"+str(c))
                    rowsize += int(eval(etcd.get("/table/" + tabless[t] + "/columnsize/"+str(c))[0]))
                frlist.append(Fragment(f_id,tabless[t],sname2sid[sname],lenrow*rowsize,1,verti_condi=vconds))
    ...
    for n in nodes:
        if n.if_valid==0:
            ...
            continue
        if n.projection:
            dot.node(str(i),n.type+' '+' '.join(n.projection),color=color_dict[sname2sid[n.site]])
        if n.select_condi:
            dot.node(str(i),n.type+' '+' '.join(n.select_condi),color=color_dict[sname2sid[n.site]])
        if n.type=='join':
            dot.node(str(i),n.type+' '+str(n.join),color=color_dict[sname2sid[n.site]])
        if n.f_id!=None:
            dot.node(str(i),n.type+' '+str(n.f_name)+' '+str(n.site),color=color_dict[sname2sid[n.site]])
        if n.type=='union':
            dot.node(str(i),'union',color=color_dict[sname2sid[n.site]])
        if n.children!=[]:
            for c in n.children:
