...
from client.structures import Attribute,Selection,Join,Fragment,Node,Table
...
def parsesql(query):    
            ...
            if token.get_parent_name():
                ...
                for i in range(len(all_need_attributes)):
                    if '.' not in all_need_attributes[i]:
                        all_need_attributes[i] = token.value + '.' + all_need_attributes[i]
        ...
        if isinstance(token,sqlparse.sql.IdentifierList):
            for iden in token.get_identifiers():
                if iden.get_parent_name():
                    ...
                    for i in range(len(all_need_attributes)):
                        if '.' not in all_need_attributes[i]:
                            all_need_attributes[i] = iden.value + '.' + all_need_attributes[i]
            #print(token.value, token.get_real_name(),token.get_parent_name())
                        ...
                        if isinstance(selectitem,sqlparse.sql.Identifier):
                            if selectitem.get_parent_name() is None:
                                ...
                                if afterequal==0:
                                    ...
                                    sel_or_join = 1
                        elif isinstance(selectitem,sqlparse.sql.Token) and selectitem.value in ['>','=','<']:
                            select[1] = selectitem.value
                            afterequal=1
                        elif isinstance(selectitem,sqlparse.sql.Token) and selectitem.value.isdigit():
                            select[2] = selectitem.value
                        elif isinstance(selectitem,sqlparse.sql.Token) and selectitem.value:
                            select[2] = selectitem.value
                    if sel_or_join==0:
    ...
    '''
    return if_project_all, selects, joins, all_need_attributes, tables, final_attributes, exclude_select_attributes, join_extra_selects
...
def create_a_tree(sql):
    ...
    for attr in etcd.get_prefix("/attrinfo"):
        ...
        if tableinfo['type'] in ['U','N']:
            statistics[tableinfo['attr']] = [tableinfo['type']]+[int(a) for a in tableinfo['numbers']]
                ...
                if '.'.join([o.attr.table,o.attr.attr])==select_condition[0]:
                    ...
                    if info[0]=='U':
                        if o.operator in ['<','<='] and select_condition[1] in ['<','<=']:
                            return origin_size*(o.num_value-info[1])/(selectnum-info[1])
                        if o.operator in ['>','>='] and select_condition[1] in ['<','<=']:
                            return origin_size*(selectnum-o.num_value)/(info[2]-o.num_value)
                        if o.operator in ['<','<='] and select_condition[1] in ['>','>=']:
                            return origin_size*(o.num_value-selectnum)/(o.num_value-info[1])
                        if o.operator in ['>','>='] and select_condition[1] in ['>','>=']:
                            #print('finddddddddddddddddd!!!!!!!!!!!!!!!!!!!!!!')
                            #print(origin_size,info[2],o.num_value,selectnum)
                            return origin_size*(info[2]-selectnum)/(info[2]-o.num_value)
        if chonghe==0 or origin_range==None:
            ...
            if info[0]=='N':
                ...
                for i in range(info[3],info[4]+1):
                    seppro[i-info[3]] = 1/math.sqrt(2*3.14*info[2])*math.exp(-(i-info[1])*(i-info[1])/(2*info[2]))
                seppro = list(np.array(seppro)/np.sum(seppro))

                #print(seppro)
                if select_condition[1] == '>':
                    return origin_size*(sum(seppro[selectnum+1-info[3]:info[4]+1-info[3]]))
                if select_condition[1] == '>=':
                    return origin_size*(sum(seppro[selectnum-info[3]:info[4]+1-info[3]]))
                if select_condition[1] == '<':
                    #print("进来了！！！")
                    #print(sum(seppro[info[3]-info[3]:selectnum-info[3]]))
                    return origin_size*(sum(seppro[info[3]-info[3]:selectnum-info[3]]))
                if select_condition[1] == '<=':
                    return origin_size*(sum(seppro[info[3]-info[3]:selectnum+1-info[3]]))
    ...
    for t in range(len(tabless)):
        ...
        for c in cinfo:
            columnsize = eval(etcd.get("/table/" + tabless[t] + "/columnsize/"+str(c))[0])
            #print(columnsize)
    ...
    if_project_all, selects, joins, all_need_attributes, tables,final_attributes,exclude_select_attribute,join_extra_selects = parsesql(sql)
        ...
        for i in etcd.get_prefix("/table/" + tabless[t] + "/fragment"):
            ...
            for a in finfo['columns']:
                if tabless[t]=='Customer' and a=='id':
                    attrl.append(Attribute('Customer','id',f_id=f_id,if_join_attr=True))
                else:
            ...
            if finfo["conditions"]!='':
                ...
                for comp in conditionlist:
                    ...
                    if right.isdigit():
                        ...
                        hconds.append(Selection(compattr, op, f_id, s_id, num_value = int(right)))
                    else:
                        #print('Selection',compattr.table,compattr.attr,op,f_id,s_id,right)
                        hconds.append(Selection(compattr, op, f_id, s_id, str_value = right))
            else:
                for at in attrl:
                    if at.attr in finfo['columns']:
                        vconds.append(at)
            if finfo["conditions"]!='':
                lenrow = int(etcd.get("/table/" + tabless[t] + "/lenfragment/"+str(s_id))[0])
                rowsize = 0
                cinfo = eval(etcd.get("/table/" + tabless[t] + "/columns")[0])
                cinfo = [c[0] for c in cinfo]
                for c in cinfo:
                    #print("/table/" + tabless[t] + "/columnsize/"+str(c))
                    rowsize += int(eval(etcd.get("/table/" + tabless[t] + "/columnsize/"+str(c))[0]))
                frlist.append(Fragment(f_id,tabless[t],s_id,lenrow*rowsize,0,hori_condi=hconds))
                ...
                for c in finfo['columns']:
                    #print("/table/" + tabless[t] + "/columnsize/"+str(c))
                    rowsize += int(eval(etcd.get("/table/" + tabless[t] + "/columnsize/"+str(c))[0]))
                frlist.append(Fragment(f_id,tabless[t],s_id,lenrow*rowsize,1,verti_condi=vconds))
    ...
    #print('frrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrlist',len(fragmentlist))
    ...
    for t in tables:
        for frag in table_map[t].frag_list:
            ...
            if frag.hor_or_ver==0:
                n = Node(len(nodes),'fragment',-1,[],[t],frag.s_id,frag.size, columns=table_map[t].attributes,#列
                                f_id=frag.id, f_name=frag.table,top=len(nodes))
                nodes.append(n)
            else:
                n = Node(len(nodes),'fragment',-1,[],[t],frag.s_id,frag.size, columns=['.'.join([at.table,at.attr]) for at in frag.verti_condi],#列
                                f_id=frag.id, f_name=frag.table,top=len(nodes))
                nodes.append(n)
            sitenums[n.site-1][f_id_table_id[n.f_id]] = n.size
            '''
            if n.f_id in[0,1,2,3]:
                sitenums[n.site-1][0] = n.size
            if n.f_id in[4,5,6]:
                sitenums[n.site-1][1] = n.size
            if n.f_id in[7,8]:
                sitenums[n.site-1][2] = n.size
            if n.f_id in[9,10,11,12]:
                sitenums[n.site-1][3] = n.size
            '''
    #print()
    if len(selects+join_extra_selects)>0:
        for s in selects+join_extra_selects:                           #query中涉及到的属性
            if s[0] in select_attr:                      #如果涉及的属性在分片属性里
                #print(s[0])
                frs = [i for i in range(len(select_attr)) if select_attr[i]==s[0]]  #找到那个属性
                #print(frs)
                for j in frs:
                    if selectionlist[j].num_value:
                        if not operator_conflict(s[1],selectionlist[j].operator,float(s[2]),selectionlist[j].num_value): #如果不冲突
                            #print(s)
                    ...
                    if selectionlist[j].str_value:
                        #print("找到了！！！！！！！！！！！！！！！！")
                        if not operator_conflict(s[1],selectionlist[j].operator,s[2].strip("'"),selectionlist[j].str_value): #如果不冲突
                            #print(s)
    ...
    for n in nodes:                       #剪掉vertical
        if fragmentlist[n.f_id].hor_or_ver==1:
            #print(n.f_id)
            include_attrs = ['.'.join([v.table,v.attr]) for v in fragmentlist[n.f_id].verti_condi if v.if_join_attr==False]
            ...
            for v in fragmentlist[n.f_id].verti_condi:
                if '.'.join([v.table,v.attr]) in need_select_attrs:
                    ...
                    if selects[index][0] in statistics.keys():
                        new_size = int(count_size(n.size,select_condition=selects[index]))     #vertical按理来说本身不会有范围
                    new = Node(nodes[len(nodes)-1].id+1,'select',
                                parent = -1,
                                children = [n.id],
                                tables=n.tables,
                                site = n.site,
                    ...
                    nodes.append(new)
                    if '.'.join([v.table,v.attr])==special_attr and special_attr not in final_attributes:
                        new_size = int(count_pro_size(new.size,n.columns,['Customer.rank']))  #如果rank不需要就剪掉
                        #print("这里project了嘛？？？？？")
                        new1 = Node(nodes[len(nodes)-1].id+1,'projection',
                                parent = -1,
                                children = [new.id],
                                tables=n.tables,
                                site = n.site,
        ...
        if n.f_id!=None and fragmentlist[n.f_id].hor_or_ver==0:    #horizontal的select
            ...
            if fragmentlist[n.f_id].table in need_select_tables:
                ...
                for i in range(len(origin_range)):
                    #print(selects[index])
                    if selects[index][0]==origin_range[i].attr.table+'.'+origin_range[i].attr.attr and selects[index][1]==origin_range[i].operator and (selects[index][2].strip("'")==origin_range[i].str_value or (selects[index][2].isdigit() and int(selects[index][2])==origin_range[i].num_value)):
                        noneedselect=1
                if noneedselect==0:
                    new_size = n.size
                    if selects[index][0] in statistics.keys():
                        new_size = int(count_size(n.size,
                                origin_range=fragmentlist[n.f_id].hori_condi,
            ...
            if fragmentlist[n.f_id].table in tables:
                all_this_need_attributes = [at for at in exclude_select_attribute if at.startswith(fragmentlist[n.f_id].table)]
                ...
                if len(all_this_need_attributes)<len(table_map[fragmentlist[n.f_id].table].attributes):
                    ...
                    origin_attrs = [a for a in table_map[fragmentlist[n.f_id].table].attributes]
                    new_size = int(count_pro_size(n1.size,origin_attrs,all_this_need_attributes))
                    sitenums[n.site-1][f_id_table_id[n.f_id]] = new_size
                    new = Node(nodes[len(nodes)-1].id+1,'projection',
                            parent = -1,
                            children = [],
                            tables=n.tables,
                            site = n.site,
    ...
    #    print(f.hor_or_ver)
    verticals = [n for n in nodes if n.type=='fragment' and fragmentlist[n.f_id].hor_or_ver==1 and n.if_valid==True]
    #print('verticalslen',verticals)
    if len(verticals)>1:
        joinattr = ['.'.join([v.table,v.attr]) for v in fragmentlist[verticals[0].f_id].verti_condi if v.if_join_attr==True]
        #print(joinattr)
        new = Node(nodes[len(nodes)-1].id+1,'join',
                    parent = -1,
                    children = [],
    ...
    if len(joins)>0:
                        ...
                        if f2.id in exist_fids and f2.hor_or_ver==0:
                                    ...
                                    for s2 in f2.hori_condi:
                                        if s2.attr.attr==j[2].split('.')[1]:
                                            ...
                                            if s1.num_value and s2.num_value:
                                                ...
                                                if not operator_conflict(s1.operator,s2.operator,s1.num_value,s2.num_value):
                                                    #print(f1.id,f2.id)
                                                    #print(s1.attr.attr,s1.operator,s1.num_value)
                                                    #print(s2.attr.attr,s2.operator,s2.num_value)
                                                    conflict_pairs.append([f1.id,f2.id])
                                                    if_conflict = 1
                                            if s1.str_value and s2.str_value:
                                                if not operator_conflict(s1.operator,s2.operator,s1.str_value,s2.str_value):
                                                    conflict_pairs.append([f1.id,f2.id])
                                                    if_conflict = 1
                            if if_conflict==0:
                            ...
                            if t2_vertical_already==0:
                                ...
                                for n in nodes:
                                    if n.f_id==f1.id:
                                        n1 = n
                                        while n1.parent!=-1 and nodes[n1.parent].type!='join':
                                            n1 = nodes[n1.parent]
                                    if n.f_id==f2.id:
                                        n2 = n
                                        while n2.parent!=-1 and nodes[n2.parent].type!='join':
                                            n2 = nodes[n2.parent]
                                if join_index==0:
                                    all_pairs.append([f1.id,f2.id])
                                    #print(all_pairs)
        ...
        for c in conflict_pairs:
            if f_id_table_id[c[1]]==Rp:
                if f_id_s_id[c[1]] not in use_conflict_pairs.keys():
                    use_conflict_pairs[f_id_s_id[c[1]]] = [[f_id_s_id[c[0]],f_id_table_id[c[0]]]]
                else:
                    use_conflict_pairs[f_id_s_id[c[1]]].append([f_id_s_id[c[0]],f_id_table_id[c[0]]])
        ...
        for i in range(len(all_pairs)):
            ...
            if on_site:
                nodes_on_sites[maxsite-1].append(all_pairs[i])    #一个组合里，传到被当作processing set的那个站点上
            else:
                #print()
                nodes_on_sites[process_sites[0]].append(all_pairs[i])
            #print(nodes_on_sites)
            #break
        table_slices = [list(set(np.array(all_pairs)[:,i])) for i  in range(len(all_pairs[0]))]
        ...
        for i in range(len(nodes_on_sites)):
            ...
            #for f in range(len(for_join)):
                if len(for_join[f])==1:                    #没有join的剪枝
                        ...
                        for frag in wait_to_union:
                            for n in nodes:
                                if n.f_id==frag:
                                    ...
                                    break
                        new = Node(nodes[len(nodes)-1].id+1,'union',
                                    parent = -1,
                                    children = needunion,
                                    tables=[],
                                    site = i+1,          
                                    size=sum([nodes[n].size for n in needunion]),
                                    columns=nodes[needunion[0]].columns)      #待改
                        nodes.append(new)
                    ...
                    for p in range(len(all_join_patterns)):            #把不一样的这些先union再join再union
                        tabless = all_join_patterns[p].split(' ')
                        special_union_nodes = [None for _ in range(len(tabless))]
                        for t in range(f,f1):
                            if '[' in tabless[t]:
                                #print("应当是B和O",tabless[t])
                                needunion = []              #所有分片union起来
                                wait_to_union = json.loads(tabless[t])
                                for frag in wait_to_union:
                                    for n in nodes:
                                        if n.f_id==frag:
                                            needunion.append(n.top)
                        ...
                        for u in range(f+1,f1):
                            ...
                            new1 = Node(nodes[len(nodes)-1].id+1,'join',      #再join
                                        parent = -1,
                    ...
                    new2 = Node(nodes[len(nodes)-1].id+1,'union',           #再union
                                parent = -1,
                                children = [s.id for s in special_join_nodes],
                                tables=[],
                                site = i+1,          
                                size=sum([s.size for s in special_join_nodes]),
                                columns=special_join_nodes[0].columns)      #待改
                    nodes.append(new2)
            ...
            for u in range(1,len(union_nodes)):
                ...
                if iter_ >=len(ad_joins):
                    break
                n2 = union_nodes[u]
                new1 = Node(nodes[len(nodes)-1].id+1,'join',      #再join   joins[joins]
                            parent = -1,
                            children = [previous_node.id, n2.id],
                            tables=[previous_node.tables+n2.tables],
                            site = i+1,          #待改
            ...
            if change:
                #print("previous_node",previous_node.columns)
                #print("projection",projection)
                newsize = int(count_pro_size(previous_node.size,previous_node.columns,final_attributes))
                newp = Node(nodes[len(nodes)-1].id+1,'projection',      #再join   joins[joins]
                        parent = -1,
                        children = [previous_node.id],
                        tables=previous_node.tables,
    ...
    for n in nodes:
        if n.if_valid==0:
            ...
            continue
        if n.projection:
            dot.node(str(i),n.type+' '+' '.join(n.projection),color=color_dict[n.site])
        if n.select_condi:
            dot.node(str(i),n.type+' '+' '.join(n.select_condi),color=color_dict[n.site])
        if n.type=='join':
            dot.node(str(i),n.type+' '+str(n.join),color=color_dict[n.site])
        if n.f_id!=None:
            dot.node(str(i),n.type+' '+str(n.f_name)+' '+str(n.site),color=color_dict[n.site])
        if n.type=='union':
            dot.node(str(i),'union',color=color_dict[n.site])
        if n.children!=[]:
            for c in n.children:
        ...
        i += 1
    #dot.edges(edges)
    dot.render(filename="tree.svg", directory="",view=False)
    #print(sitenums)
