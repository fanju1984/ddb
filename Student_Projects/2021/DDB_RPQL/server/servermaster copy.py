...
def subclient(ip, port, db):
    return client.pop(ip + ":" + port + ":" + db)


class servicer(net_pb2_grpc.NetServiceServicer):
    def __init__(self):
        #self.conndb = None
        self.conndb = Conndb(user=_MYSQL_USER, password=_MYSQL_PASSWORD, database=_DB)

    def createdatabase(self, request, context):
        dbname = request.dbname + _PORT
        conn = pymysql.connect(host='localhost',user=_MYSQL_USER,password=_MYSQL_PASSWORD,charset='utf8mb4')
        # 创建游标
        cursor = conn.cursor()
    ...
    def usedatabase(self, request, context):
        ...
        for i in etcdsites:
            ip, port, db = i.split(":")
            newsites.append(":".join(ip, port, _DB))
    ...
    def Loaddata(self, request, context):
        ...
        for i in range(fragmentlen):
            ...
            etcd.put("/table/{}/columnsize/{}".format(table_name, columnname), str(size))
        for i in range(lendata):
            # i 目前是单独的一行数据
            # 需要获取一下fragment的信息
    ...
    def Insertdata(self, request, context):
        ...
        table_name = request.table_name
        fragment_columns = request.fragment_columns
        fragmentlen = len(eval(fragment_columns))
        bytedata = request.data
        data = pickle.loads(bytedata)
        # table_name = "Book"
        columns = eval(etcd.get("/table/{}/columns".format(table_name))[0])
    ...
    def SimpleSelect(self, request, context):
        ...
        bytedata = pickle.dumps(data)
        return net_pb2.SimpleSelectReturn(status="to delete data's key", key=key, data=bytedata)
    ...
    def jr_grpc_test(self, request, context): #jr测试grpc远程过程调用
        ...
        try:
            ...
            response = net_conn.jr_grpc_test(net_pb2.para_jr_grpc_test(th=th+1, site=nex_site))
            res = response.res
    ...
    def start_jr(self, request, context):
        ...
        return net_pb2.ret_start_jr(res=restr) #eval解析出(columns,data,trans_vol) data=tuple(tuple)
...
def jr_siteofme():
    global _THIS
    site2ipd = { '10.77.70.61:8883:db1':1,
