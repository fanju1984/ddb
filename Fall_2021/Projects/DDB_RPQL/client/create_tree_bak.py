...
def create_a_tree(sql):
    ...
    for n in nodes:
        if n.if_valid==0:
            ...
            continue
        if n.projection:
            dot.node(str(i),str(i)+' '+n.type+' '+' '.join(n.projection),color=color_dict[n.site])
        if n.select_condi:
            dot.node(str(i),str(i)+' '+n.type+' '+' '.join(n.select_condi),color=color_dict[n.site])
        if n.type=='join':
            dot.node(str(i),str(i)+' '+n.type+' '+str(n.join),color=color_dict[n.site])
        if n.f_id!=None:
            dot.node(str(i),str(i)+' '+n.type+' '+str(n.f_name)+' '+str(n.site),color=color_dict[n.site])
        if n.type=='union':
            dot.node(str(i),str(i)+' '+'union',color=color_dict[n.site])
        if n.children!=[]:
            for c in n.children:
